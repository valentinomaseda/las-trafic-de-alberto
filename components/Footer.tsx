...
import { PhoneCall, MapPin, Clock3, ArrowUpRight } from 'lucide-react';
  ...
  return (
    <footer className="border-t border-[#e8e9ed] bg-white">
      <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr_0.9fr]">
          <div>
            <Link href="#top" className="inline-flex items-center gap-3">
              ...
              <span>
                <span className="block text-lg font-semibold text-slate-900">Las Trafic de Alberto</span>
                <span className="block text-sm text-slate-500">Viajes grupales, traslados y excursiones</span>
              </span>
            </Link>
          ...
          <div>
            ...
            <div className="mt-5 grid gap-3 text-sm font-medium">
              {quickLinks.map((item) => (
                <Link key={item.label} href={item.href} className="inline-flex items-center gap-2 text-slate-700 transition hover:text-[#da0200]">
        ...
        <div className="mt-12 flex flex-col gap-3 border-t border-[#e8e9ed] pt-6 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
